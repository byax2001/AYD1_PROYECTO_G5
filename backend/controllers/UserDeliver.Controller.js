            ...
            if (err)throw err;
                ...
                database.query(querysMySQL.list_users_byuser,[user.username],function(err,result,fields){    
                    if (err)throw err;
                        ...
                        database.query(querysMySQL.ins_sol, [currentDate,user.nombre, user.apellido,user.email,user.nit,user.medio_transporte,user.userid,user.descripcion,user.username,passwordEncrypt,user.tipo_empresa,user.municipio,user.tipo_licencia,0,user.telefono,user.direccion],async function(err,result,fields){
